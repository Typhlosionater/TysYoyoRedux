		...
		public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
		{
			...
			if (OnHitEffectCooldown <= 0)
			{
				...
				for (int i = 0; i < ProjectileAmount; i++)
				{
					...
					float num569 = (0f - VariableMomentum.X) * Main.rand.Next(40, 70) * 0.01f +
					               Main.rand.Next(-20, 21) * 0.4f;
					float num570 = (0f - VariableMomentum.Y) * Main.rand.Next(40, 70) * 0.01f +
					               Main.rand.Next(-20, 21) * 0.4f;
					Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center,
						new Vector2(num569, num570),
						ModContent.ProjectileType<NewYoyoEffects.PrismaticThrowPrismaticShardProjectile>(),
						Projectile.damage / 4, 0f, Projectile.owner);
				}
